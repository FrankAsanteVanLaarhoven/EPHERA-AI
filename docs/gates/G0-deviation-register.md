...
Closed at G3 — see [`G3-report.md`](G3-report.md).
...
| D-17 | S2 | **Closed.** The console route that flipped in-memory flags is deleted. An approved `wallet.freeze` now calls the ledger and genuinely freezes the account, with the operator and approved change recorded in evidence. Actions with no owning service — kill switch, feature flags, provider approval, mandates — return 501 instead of reporting success, so nothing is claimed as applied when it was not. The control plane now owns platform flags, and the payment orchestrator reads them: an approved kill switch genuinely stops sends at prepare and at submit. Resuming needs a second operator too. Failure is asymmetric by design — once stopped, an unreachable control plane keeps it stopped; a healthy platform keeps running but reports it is on a stale value; a cold start that has never reached the control plane refuses | `services/platform-control-bff/internal/effect/`, `services/payments/internal/flags/` | G8 |
